...
export default function AccountPage() {
  ...
  const recentOrder = orders.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0]
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {accountCards.map((card) => (
          <Card key={card.title} className="hover:shadow-md transition-shadow dark:border-border">
      ...
      {recentOrder && (
        <div className="space-y-4">
          ...
          <div className="rounded-lg border bg-card dark:bg-card p-4 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h3 className="font-medium">Order #{recentOrder.orderNumber}</h3>
                <p className="text-sm text-muted-foreground dark:text-muted-foreground">
                  {new Date(recentOrder.date).toLocaleDateString()} • {recentOrder.items.length}{" "}
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium">${recentOrder.total.toFixed(2)}</span>
              </div>
            </div>
            <div className="mt-4 flex flex-col sm:flex-row gap-2 sm:items-center justify-between">
              <div className="flex flex-wrap gap-2">
                {recentOrder.items.slice(0, 3).map((item, index) => (
                  <div key={index} className="h-10 w-10 rounded-md bg-muted dark:bg-muted overflow-hidden">
                    <img
